...
bool OnnxSTTImpl::loadVocabulary(const std::string& path) {
...
bool OnnxSTTImpl::loadCMVNStats(const std::string& path) {
    std::ifstream file(path);
    if (!file.is_open()) {
...
std::vector<float> OnnxSTTImpl::extractFbankFeatures(
...
void OnnxSTTImpl::applyCMVN(std::vector<float>& features) {
...
std::string OnnxSTTImpl::ctcDecode(const std::vector<float>& logits, 
