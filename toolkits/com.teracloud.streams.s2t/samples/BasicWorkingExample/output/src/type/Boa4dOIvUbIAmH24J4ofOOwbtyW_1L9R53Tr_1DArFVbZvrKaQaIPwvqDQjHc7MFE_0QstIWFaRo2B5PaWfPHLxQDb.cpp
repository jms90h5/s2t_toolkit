...
#include "Boa4dOIvUbIAmH24J4ofOOwbtyW_1L9R53Tr_1DArFVbZvrKaQaIPwvqDQjHc7MFE_0QstIWFaRo2B5PaWfPHLxQDb.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring websocketUrl>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("websocketUrl");
        TypeOffset t(MY_OFFSETOF(websocketUrl_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "websocketUrl") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, websocketUrl_);
   else
     istr >> websocketUrl_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, websocketUrl_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "websocketUrl=", get_websocketUrl()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_websocketUrl());
