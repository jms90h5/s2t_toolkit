...
#include "BFnw1L8VHTxQVFSKqgwlm0fQwKSYjgcr1gowQXqI29YclLU3jp63B_1ncbCBOZPDW0UpaMIFMq2yz9b9B8KqeGAm.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring text,boolean isFinal,float64 confidence,uint64 audioTimestamp,uint64 latencyMs>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("latencyMs");
        TypeOffset t(MY_OFFSETOF(latencyMs_, tm), 
                     Meta::Type::typeOf<SPL::uint64 >(), 
                     &typeid(SPL::uint64));
        addMapping(*tm, t, s, 4);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "latencyMs") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, latencyMs_);
   else
     istr >> latencyMs_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, latencyMs_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "latencyMs=", get_latencyMs()) ;
    ostr << '}';
}
