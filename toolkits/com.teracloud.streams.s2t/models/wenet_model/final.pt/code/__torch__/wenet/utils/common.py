def reverse_pad_list(ys_pad: Tensor,
  ...
  for _3 in range(ops.prim.min(_2)):
    ...
    _4 = torch.slice(torch.to(y, 3), 0, None, annotate(int, i))
...
def add_sos_eos(ys_pad: Tensor,
  ...
  _sos = torch.tensor([sos], dtype=4, device=ops.prim.device(ys_pad))
  _eos = torch.tensor([eos], dtype=4, device=ops.prim.device(ys_pad))
  ...
  for _9 in range(torch.len(ys)):
    y0 = ys[_9]
    _10 = torch.append(ys_in, torch.cat([_sos, y0]))
  ys_out = annotate(List[Tensor], [])
  for _11 in range(torch.len(ys)):
    y1 = ys[_11]
    _12 = torch.append(ys_out, torch.cat([y1, _eos]))
...
def pad_list(xs: List[Tensor],
  ...
  pad = torch.zeros([n_batch, max_len], dtype=_22, layout=None, device=_23)
  pad0 = torch.fill_(pad, pad_value)
  for i in range(n_batch):
    _24 = xs[i]
    _25 = torch.slice(torch.select(pad0, 0, i), 0, None, torch.size(xs[i], 0))
    _26 = torch.copy_(_25, _24)
  return pad0
